...
export default function ReadingPart6Page() {
  ...
  useEffect(() => {
    apiFetch("/questions?part=PART6&limit=8")
      .then((r) => r.json())
  ...
  // Split content: passage is before the last \n\n[N]
  const parts = q?.content?.split(/\n\n\[(\d+)\]/) || [];
  const passage = parts[0] || q?.content || "";
  const questionLabel = parts.length > 1 ? parts[parts.length - 1] : "";
  ...
  if (!q) return (
    <div className="p-6 max-w-xl mx-auto text-center py-20">
      ...
      <h2 className="text-xl font-bold mb-2">Part 6: Điền vào đoạn văn</h2>
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-3">
        <h1 className="text-xl font-bold">📝 Part 6: Điền vào đoạn văn</h1>
      ...
      <div className="card p-5 mb-4" style={{ background: "hsl(var(--muted)/0.4)" }}>
        ...
        <pre className="text-sm whitespace-pre-wrap font-sans leading-relaxed">{passage}</pre>
      </div>

      <div className="card p-5 mb-4">
        <p className="text-sm font-medium mb-4">{questionLabel || "Chọn từ đúng nhất:"}</p>
